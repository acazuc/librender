...
namespace librender
{
	...
	void DrawableBatch::updateTexCoords()
	{
		...
		for (size_t i = 0; i < this->childs.size(); ++i)
		{
			...
			if (this->mustResize || child->getChanges() & DRAWABLE_UPDATE_TEX_COORDS)
			{
				std::copy(child->getTexCoords().begin(), child->getTexCoords().end(), this->texCoords.begin() + count);
				child->removeChanges(DRAWABLE_UPDATE_TEX_COORDS);
			}
	...
	void DrawableBatch::updateVertexes()
	{
		...
		{
			DrawableBatched *child = this->childs[i];
			if (this->mustResize || child->getChanges() & DRAWABLE_UPDATE_VERTEXES)
			{
				std::copy(child->getVertexes().begin(), child->getVertexes().end(), this->vertexes.begin() + count);
				child->removeChanges(DRAWABLE_UPDATE_VERTEXES);
	...
	void DrawableBatch::updateIndices()
	{
		...
		{
			DrawableBatched *child = this->childs[i];
			if (this->mustResize || child->getChanges() & DRAWABLE_UPDATE_INDICES)
			{
				...
				child->removeChanges(DRAWABLE_UPDATE_INDICES);
			}
	...
	void DrawableBatch::updateColors()
	{
		...
		{
			DrawableBatched *child = this->childs[i];
			if (this->mustResize || child->getChanges() & DRAWABLE_UPDATE_COLORS)
			{
				std::copy(child->getColors().begin(), child->getColors().end(), this->colors.begin() + count);
				child->removeChanges(DRAWABLE_UPDATE_COLORS);
			}
	...
	bool DrawableBatch::update()
	{
		...
		if (!this->verticesNumber)
			return false;
		if (this->mustResize)
			requireUpdates(DRAWABLE_UPDATE_INDICES | DRAWABLE_UPDATE_TEX_COORDS | DRAWABLE_UPDATE_VERTEXES | DRAWABLE_UPDATE_COLORS);
